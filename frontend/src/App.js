...
function App() {
    ...
    return (<div className="app">
      <Header_1.default health={health}/>
      <nav className="tabs">
        {["logs", "metrics", "traces"].map((tab) => (<button key={tab} className={`tab-btn ${activeTab === tab ? "active" : ""}`} onClick={() => setActiveTab(tab)}>
            {tab === "logs" && "📋 "}
            {tab === "metrics" && "📊 "}
            {tab === "traces" && "🔍 "}
      ...
      <main className="content">
        {activeTab === "logs" && <LogPanel_1.default apiBase={API_BASE}/>}
        {activeTab === "metrics" && <MetricsPanel_1.default apiBase={API_BASE}/>}
        {activeTab === "traces" && <TracePanel_1.default />}
      </main>
    </div>);
}
exports.default = App;
